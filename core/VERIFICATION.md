...
| 21 | Open loops (v6.0.0) | `loop.close` with no matching `loop.open` for that `loop_id` in the project — including a second close of the same loop — FAILs `loop_close_without_open`. A `loop_id` opened twice in one project FAILs `duplicate_loop_open` (loops are never reopened; a recurrence is a new `L` id). A `session.handoff` whose `open_loops` list omits a loop that is open at that point in the chain FAILs `handoff_missing_open_loops`. A `loop.open` whose `loop_id` is missing or not a string FAILs `malformed_loop_id` and opens nothing -- a loop with no usable `L` id is not a row anything can count. An **absent** `open_loops` field is a note (`handoff_open_loops_absent`), not a FAIL — pre-v6 chains keep verifying. Staleness is not checked here: it is date-dependent and belongs to `hw status`, which leads with OVERDUE OPEN LOOPS. From the field incident where a fully-gated server rejoin sat unconsumed for five weeks and surfaced through an unrelated symptom in production. See `core/SUBSTRATE.md` §Open Loops. |
| 22 | Secrets gate (v6.0.0) | Every string in every payload is scanned for secret-shaped content: credential assignments (`password=`, `api_key:`), PEM / OpenSSH private-key blocks, connection strings carrying credentials, bearer / vendor-token shapes, and unlabeled high-entropy tokens. A hit is a **WARNING**, `possible_secret_in_event` — never a FAIL by default, because historical chains contain leaked secrets and refusing to verify them unleaks nothing. `hw verify --strict-secrets` promotes it to FAIL for new-chain hygiene. The hashes the harness itself requires (citations, `sha256:` prefixes, 12-64 hex in `hash` / `prev_hash` / `sha256` / `content_sha256` / `soul_hash` / `test_ref`) and the `[REDACTED-SECRET]` marker never trip it; pure hex cannot reach the 4.0 bits/char entropy threshold by construction. Reports name the event, the field and the rule, never the value. The real enforcement is the refusal at `hw add` (`core/SUBSTRATE.md` §`hw add` step 4) — this check is the after-the-fact half, telling an operator what to rotate against on a log where deletion is not available. From the field incident where a sync digest copied a DSRM password verbatim into a mailbox and the attempted fix was a cleartext blocklist of live credentials. See `core/SUBSTRATE.md` §Secrets Gate. |

**Malformed identifiers never abort verification (v6.0.0).** A payload field that names something -- `project_id`, `loop_id`, `cycle_id`, `project` -- must be a non-blank string. One that is missing, or carries a list, an int or blank text, is a **malformed payload**: a blocking FAIL naming the event, the field and the type, reported alongside every other finding in a complete report. It is never an exception. `events.jsonl` is append-only, so the operator cannot delete the offending event; a verifier that raised on one would turn a single bad payload into a workspace that can never be verified again, and would say nothing about the rest of the chain. Every check reads identifiers through that guard, so no dict key, set member, sort, join or path is ever built out of a value that is not an id.

**Profile note (v6.0.0).** The checks above run with two relaxations under `profile: single-executor` (`core/SUBSTRATE.md` §Execution Profile). The verifier reads the profile the way it reads `lifecycle`: the active schema's `schema.yaml` first, then `PROJECT.md`, and unknown reads as the `multi-actor` default, so a project that declares nothing is unaffected. `hw verify` emits a `profile_single_executor` note so a report shows which rules are relaxed.

1. **A missing `actor` is not a malformed payload.** It defaults to `executor`, and that default is also what `hw fold` and replay-based resume match against (`core/ATOMICITY.md` §Resume Without Session State).
2. **Citation handles may drop the `#hash` suffix** — and this one has a cost the harness cannot hide. `hw verify` recognizes a citation by the `[KIND-NNN#hash]` shape, so a bare `F-012` handle is **invisible** to it: **check 2 does not run on it** (a bare handle naming a nonexistent artifact is never reported broken), **check 3 does not run on it** (it can never go stale, because there is no recorded hash to compare), and **the Ratchet cannot fire on it** (`core/ATOMICITY.md` §Ratchet — a completed task whose `consumes:` holds only bare handles is never re-blocked when a later task supersedes what it consumed).

**Keep the `#hash` on any citation you rely on across sessions.** Bare handles are for conversational references — a pointer a human reads in the same session. Anything a task consumes, an artifact depends on, or a future agent resumes from should carry the full citation, so checks 2, 3 and the Ratchet have something to inspect.

Everything else is unchanged: the chain is still hash-linked, artifact hashes are still tracked, and every check above still runs.
