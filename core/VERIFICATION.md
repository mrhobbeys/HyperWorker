...
**Profile note (v6.0.0).** The checks above run identically on every project, with one relaxation: under `profile: single-executor` (`core/SUBSTRATE.md` §Execution Profile) a missing `actor` is not a malformed payload — it defaults to `executor` — and citation handles may drop the `#hash` suffix. The verifier reads the profile the way it reads `lifecycle`: the active schema's `schema.yaml` first, then `PROJECT.md`, and unknown reads as the `multi-actor` default, so every existing project is unaffected. Nothing else changes: the chain is still hash-linked, artifact hashes are still tracked, and checks 1-22 all still run. `hw verify` emits a `profile_single_executor` note so a report shows which rules are relaxed.
