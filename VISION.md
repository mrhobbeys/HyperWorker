...
HyperWorker is a project management harness for AI agents. It makes a long-running project survive the thing that normally kills it: the agent losing the plot.

The product is six enforced mechanisms — Lock, Atomicity, Dependency, Memory, Precedence, Verification — and a boundary declaration that tells the agent which files are infrastructure and which are the project it's working on. Everything else in this repo is either a template, a reference document, or a worked case study that demonstrates the mechanisms in practice.

HyperWorker is agent-agnostic. It works with any AI agent that can read files and follow instructions — Claude Code, Cursor, Goose, Copilot, or any capable LLM. It is not tied to any vendor, IDE, or platform.

## Design Principles

**The harness is structural, not documentary.** Rules enforced by file structure and boundary declarations beat rules written in prose that an agent is supposed to remember to follow. When we face a choice between "add another paragraph to a doc" and "add a structural check the agent cannot skip," we pick the structural check.

**The planner is dumb about the domain; the config is smart.** The core mechanisms are domain-agnostic. Domain knowledge lives in the project's `config.yaml`, `00-REFERENCE-rules.md`, and task files — not in the harness. If a feature requires the harness to know whether it's running a marketing project or a software project, that feature is probably misdesigned.
...
**The execution model adapts to the platform.** Two roles — planner and executor — are fixed. How those roles map to your AI agent is adaptive. Subagent-capable platforms spawn executors. Single-agent platforms switch modes. Two-model setups assign the best model to planning. The principle is constant; the mechanism bends.

**Completion must be proven, not claimed.** Every task produces an evidence trail. Verification is structural — built into the task template — not aspirational. The ratchet principle means the project can only move forward: a task that breaks prior work is not complete, regardless of its own checklist.
...
**An orchestration layer above the planner.** If an operator needs to run many HyperWorker projects in parallel, that's a process problem, not a harness problem. We are not building a meta-harness.
