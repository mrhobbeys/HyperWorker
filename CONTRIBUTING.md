# Contributing to HyperWorker v5.1.1
...
2. **Rewrite `schema.yaml`.** Set `schema_id`, `schema_version: 1.0`, `harness_version: "5.1.1"`. Write `description` (2–3 sentences). Replace `bootstrap_questions` with questions appropriate to the domain (operating-reality fields plus any domain-specific scope declarations). Update `default_tasks.templates` to reference the new task templates.
