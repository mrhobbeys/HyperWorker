...
- **`profile: single-executor` (`core/SUBSTRATE.md` §Execution Profile, H-S12; `tools/hw-verify.py`; `templates/project-template.md`)** — measured ceremony that returned nothing on a one-agent engagement: `actor` carried the same value for ten weeks, the `event` vs `add` kind distinction was "mostly ceremony", `cite:[F-0xx#hash]` handles were "basically never consumed", and the sync-digest bridge was write-only and never read back for recovery. `profile: single-executor | multi-actor` is now declared in `PROJECT.md` (a `## Profile` section or an inline `profile:` line) and optionally in `schema.yaml`, **schema wins**; unknown or undeclared reads as `multi-actor`, so every existing project is unaffected. Under `single-executor`: `actor` is optional and defaults to `executor`, digest-bridge protocol steps are marked N/A, and citation handles may use bare ids (`F-012`) without the `#hash` suffix. **Integrity is unchanged** — the chain is still hash-linked, artifact projections still carry their hashes, `hashes.json` still tracks them, and checks 1-22 all still run; what is dropped is the citing ceremony, not the thing it was built on, so switching back to `multi-actor` costs nothing. The verifier reads the profile exactly the way it reads `lifecycle` and emits a `profile_single_executor` note so a report shows which rules are relaxed; under `multi-actor` a missing `actor` is now reported as a malformed payload, which is the behavior the format table always described. 29-case test suite.
...
Nothing outstanding. Every item scoped for the 6.0.0 release has landed.
