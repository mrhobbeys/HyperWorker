...
You have raw inputs (tweets, voice memos, originating articles, notes) and you want one piece of content distributed across three publication formats: Substack longform, X longform, YouTube lead-ins. The three are format-native, not paraphrases of each other.

The schema captures the operator's voice as a load-bearing OR field, supports an optional default lens for finding angles when none is supplied, runs an interactive interview with a hard question budget, captures verbatim "lines that sound like the operator" as their own artifact kind, and generates the three variants in a single ab-variant task with hash-citation freshness on the verbatim keepers.
...
- Three publication formats as `ab_variant_axis`
